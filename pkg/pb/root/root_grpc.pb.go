...
type CommanderClient interface {
	...
	Run(ctx context.Context, in *Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Output], error)
}
...
func (c *commanderClient) Run(ctx context.Context, in *Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Output], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Commander_ServiceDesc.Streams[1], Commander_Run_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[Empty, Output]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Commander_RunClient = grpc.ServerStreamingClient[Output]
...
type CommanderServer interface {
	...
	Run(*Empty, grpc.ServerStreamingServer[Output]) error
...
func (UnimplementedCommanderServer) Run(*Empty, grpc.ServerStreamingServer[Output]) error {
...
func _Commander_Run_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(CommanderServer).Run(m, &grpc.GenericServerStream[Empty, Output]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Commander_RunServer = grpc.ServerStreamingServer[Output]
